...
DATE_FORMAT(CAST(PF.DATA_GERACAO as Date), '%d/%m/%Y') as 'DATA DE ENVIO',
...
DATE_FORMAT(CAST(PF.DATA_VENCIMENTO as Date), '%d/%m/%Y') as 'VENCIMENTO',
CONCAT('R$ ', REPLACE(FORMAT(PF.VALOR_BRUTO, 2), ',', '.')) AS 'VALOR BRUTO DO BOLETO',
CONCAT('R$ ', REPLACE(FORMAT(PF.VALOR_LIQUIDO, 2), ',', '.')) AS 'VALOR LIQUIDO DO BOLETO',
PF.STATUS,
TFF.TIPO_DE_FLUXO AS 'TIPO DE FLUXO',
CAST(DATE_FORMAT(PF.DATA_COMPENSACAO, '%d/%m/%Y') AS Date) AS 'DATA DA COMPENSAÇÃO',
...
PF.LINK_BOLETO AS 'LINK BOLETO'

FROM T_PROPOSTAS P
INNER JOIN T_PARCELAS_FATURA PF ON PF.ID = P.FK_NOTA_FISCAL
INNER JOIN T_COMPANIES C ON C.ID = P.FK_CONTRANTE
INNER JOIN View_Datas_Boletos VDB ON VDB.TRANSACTION_ID = PF.ID
...
WHERE PF.DATA_GERACAO > DATE_SUB(CURDATE(), INTERVAL 200 DAY)
AND PF.STATUS IS NOT NULL
AND PF.STATUS != 'CANCELED'
AND C.ID = '{id}'

GROUP BY PF.ID
ORDER BY PF.DATA_VENCIMENTO DESC
    """)
