...
def establishment_finances_payment_slip(id):
    ...
    DATE_FORMAT(CAST(B.DATA_GERACAO as Date), '%d/%m/%Y') as 'DATA DE ENVIO',
    ...
    DATE_FORMAT(CAST(B.DATA_VENCIMENTO as Date), '%d/%m/%Y') as 'VENCIMENTO',
    CONCAT('R$ ', REPLACE(FORMAT(B.VALOR_BOLETO, 2), ',', '.')) AS 'VALOR DO BOLETO',
    B.STATUS,
    TFF.TIPO_DE_FLUXO AS 'TIPO DE FLUXO',
    CAST(DATE_FORMAT(B.DATA_COMPENSACAO_BOLETO, '%d/%m/%Y') AS Date) AS 'DATA DA COMPENSAÇÃO',
    ...
    B.PAYMENT_LINK AS 'LINK BOLETO'

    FROM T_PROPOSTAS P
    INNER JOIN T_BOLETOS B ON B.ID = P.TRANSACTION_ID
    INNER JOIN T_COMPANIES C ON C.ID = P.FK_CONTRANTE
    INNER JOIN View_Datas_Boletos VDB ON VDB.TRANSACTION_ID = B.ID
    ...
    WHERE B.DATA_GERACAO > DATE_SUB(CURDATE(), INTERVAL 180 DAY)
    AND B.STATUS IS NOT NULL
    AND B.STATUS != 'CANCELED'
    AND (B.CALOTE IS NULL OR B.CALOTE = 0)
    AND C.ID = {id}
